<#
    ...
    Copie tout le projet dans un nouveau dossier en remplacant les noms :
        ...
        morfanalytics         -> <nom>        (namespace, include, binaire, config)
...
Get-ChildItem -Path $Dest -Recurse -File | Where-Object {
    ...
    if ($c -match 'morfAnalytics|morfanalytics|MORFANALYTICS') {
        $c = $c -creplace 'morfAnalytics', $Camel `
                -creplace 'morfanalytics',        $Lower `
                -creplace 'MORFANALYTICS',         $Upper
        Set-Content -LiteralPath $_.FullName -Value $c -NoNewline
    }
}

# --- 3. Renommer fichiers/dossiers portant l'ancien nom ------------------
Rename-Item "$Dest\include\morfanalytics"               $Lower
Rename-Item "$Dest\config\morfanalytics.example.json"   "$Lower.example.json"
Rename-Item "$Dest\scripts\linux\morfanalytics.service" "$Lower.service"
