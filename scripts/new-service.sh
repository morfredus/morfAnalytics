...
#     morfAnalytics  -> <NomCamel>   (projet / bibliotheque / cible CMake)
#     morfanalytics         -> <nom>        (namespace, dossier include, binaire,
#                                           unite systemd, fichier de config)
#     MORFANALYTICS         -> <NOM>        (prefixe de macro)
#
# Usage :
...
grep -rlZ --binary-files=without-match \
     -e 'morfAnalytics' -e 'morfanalytics' -e 'MORFANALYTICS' \
     "$DEST" 2>/dev/null | while IFS= read -r -d '' f; do
    case "$f" in */third_party/morf/beacon/*) continue;; esac  # ne pas toucher au code vendore
    sed -i -e "s/morfAnalytics/$CAMEL/g" \
           -e "s/morfanalytics/$LOWER/g" \
           -e "s/MORFANALYTICS/$UPPER/g" "$f"
done

# --- 3. Renommer les fichiers/dossiers portant l'ancien nom --------------
mv "$DEST/include/morfanalytics"                 "$DEST/include/$LOWER"
mv "$DEST/config/morfanalytics.example.json"     "$DEST/config/$LOWER.example.json"
mv "$DEST/scripts/linux/morfanalytics.service"   "$DEST/scripts/linux/$LOWER.service"
