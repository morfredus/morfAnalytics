<#
    install-service.ps1 — Installe morfAnalytics en service Windows.
    ...
    sont copies dans un dossier fixe (par defaut C:\ProgramData\morfanalytics),
    ...
    (ou fournir -BinPath vers un morfanalytics.exe deja compile).
#>
param(
...
if (-not $BinPath) {
    $candidates = @(
        "$repoRoot\build-mingw\service\morfanalytics.exe",
        "$repoRoot\build\service\morfanalytics.exe"
    )
...
Copy-Item $BinPath "$AppDir\morfanalytics.exe" -Force

$cfgDst = "$AppDir\morfanalytics.json"
if (-not (Test-Path $cfgDst)) {
    Copy-Item "$repoRoot\config\morfanalytics.example.json" $cfgDst
...
$action    = New-ScheduledTaskAction  -Execute "$AppDir\morfanalytics.exe" `
