...
#
# morfAnalytics n'a qu'UN fichier de configuration :
#
#   config/morfanalytics.json  ->  /etc/morfsystem/morfanalytics/morfanalytics.json
#       Reglages du service : port, adresse d'ecoute, et la liste des modules
#       (analytics/photo/monitor) avec leurs sources. Lu par morfAnalytics seul.
#
...
# change une source (MeteoHub, morfPhoto, un morfMonitor) -- c'est ce script qu'il faut.
#
# La config REELLE du depot (config/morfanalytics.json) sert de reference si elle
# existe ; sinon l'exemple (config/morfanalytics.example.json). Gardez donc un vrai
# fichier dans votre clone, avec VOS sources, pour qu'il devienne la reference
...
APP_DIR="${MORF_APP_DIR:-/opt/morfanalytics}"
CONFIG_DIR="${MORF_CONFIG_DIR:-/etc/morfsystem/morfanalytics}"
SERVICE_NAME="${MORF_SERVICE_NAME:-morfanalytics}"
STATUS_PORT="${MORF_STATUS_PORT:-8799}"
...
SRC="$(pick_source morfanalytics)"
if [[ -z "$SRC" ]]; then
    echo "Aucune configuration dans $REPO_ROOT/config/ (ni morfanalytics.json ni l'exemple)." >&2
    exit 1
...
deploy_one "$SRC" "$CONFIG_DIR/morfanalytics.json" "Configuration du service   ->  $CONFIG_DIR"
...
if (( RESTART )) && command -v systemctl >/dev/null 2>&1; then
    ...
    echo
    echo "Verifier :"
    echo "    curl -s http://127.0.0.1:$STATUS_PORT/status | head -c 200"
