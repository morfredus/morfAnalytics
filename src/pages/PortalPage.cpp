...
QByteArray PortalPage::render(const QJsonArray& siteWatchReports) {
    ...
    return QStringLiteral("<!doctype html><meta charset=utf-8><title>morfAnalytics</title><style>body{font:16px system-ui;margin:3rem;max-width:60rem}li{margin:.8rem 0}.muted{color:#667085}</style><h1>morfAnalytics</h1><p>Analyses avancées disponibles.</p><ul><li><a href='/meteohub'>Analyses MeteoHub</a> — données météo</li><li><a href='/sitewatch'>Analyses SiteWatch</a> — données de journaux Web</li><li><a href='/photo'>Analyses Photo</a> — photothèque (morfPhoto)</li><li><a href='/monitor'>Analyse des machines</a> — historique du parc (morfMonitor)</li></ul><p class='muted'>%1</p>").arg(status.toHtmlEscaped()).toUtf8();
}
} // namespace morfanalytics::pages
