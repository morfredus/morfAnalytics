...
Copie autonome de la bibliothèque partagée de morfredus utilisée par morfAnalytics :

- `beacon/` — **morfBeacon** : supervision LAN (heartbeat UDP « je suis actif » +
  endpoint HTTP `/status`). morfAnalytics s'en sert pour s'annoncer sur le réseau.

> morfAnalytics n'embarque **que** morfBeacon (contrairement à ComponentHub, qui
